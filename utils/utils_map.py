...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec) > 0:
                F1_value = F1[score_threhold_idx]
                recall_value = rec[score_threhold_idx]
                precision_value = prec[score_threhold_idx]
                F1_text = "{0:.2f}".format(F1_value) + " = " + class_name + " F1 "
            ...
            all_recall.append(recall_value)
            all_precision.append(precision_value)
        ...
        # Calculate average F1, Recall, and Precision
        avg_F1 = np.mean(all_F1)
        avg_recall = np.mean(all_recall)
        avg_precision = np.mean(all_precision)
        all_metrics_text = "mAP = {0:.2f}%      ||    All_score_threhold={1} : F1={2:.2f} ; All_Recall={3:.2f}% ; All_Precision={4:.2f}%".format(
            mAP*100, score_threhold, avg_F1, avg_recall*100, avg_precision*100
        )
        print(all_metrics_text)
